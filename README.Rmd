...
where $m$ is the number of knots $s_j$ being used to discretize the integral over [0,1] and $w_j$ are quadrature weights. The parameters \code{ngrid}, \code{gridpit}, and \code{hessian} which will follow are only relevant when \code{discretize = TRUE}.
